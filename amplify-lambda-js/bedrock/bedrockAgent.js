...
export const streamAgentResponse = async (responseStream, writable, requestId) => {
    try {
        ...
        for await (const event of responseStream) {
            eventCount++;
            logger.debug(`Processing event ${eventCount}:`, { 
            ...
            if (event.chunk) {
                ...
                if (chunk.bytes) {
                    ...
                    logger.debug(`Decoded text from chunk ${chunkCount}:`, {
                        textLength: text.length,
                        textPreview: text.substring(0, 100),
                        bytesLength: chunk.bytes.length
                    });
                    ...
                    logger.debug(`Chunk ${chunkCount} has no bytes`);
                }
            }
        ...
        logger.info(`Agent response streaming summary:`, {
